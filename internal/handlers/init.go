...
func NewHandler(rd *state.RedisStore, db *database.Postgres, token, kieAPIKey, kieCallbackURL, channelID, channelLink string, kieTasks chan int64, kieAcquire func(), youKassaID, youKassaSecretKey, youKassaReturnURL string) *Handler {
	return &Handler{
		Rd:                 rd,
		Db:                 db,
		Token:              token,
